...
スタンフォード大学のエリック・ブリニョルフソン教授らによる最新調査「Canaries in the Coal Mine?」は、AIが労働市場、特に若年層の初職（エントリーレベル）に深刻な影響を与えていることを示しました。主な発見は以下の4点です。

- **若年層雇用の大幅な減少**: AI露出度の高い職種に従事する22〜25歳の雇用レベルは、露出度の低い職種と比較して19%も低くなっており、前年の13%から格差が拡大しています。
- **採用抑制のメカニズム**: この現象は解雇の増加ではなく、企業が新規採用を控える「キャリアの入り口の閉鎖」によって引き起こされています。
- **知識の性質による差異**: 教科書的な「形式知」に頼る仕事はAIに代替されやすく、実務経験で培われる「暗黙知」を要する仕事は耐性を示しています。
- **自動化か拡張か**: 業務を完全に代行する「自動化型」のAI活用は雇用を奪う一方、人間の能力を補完する「拡張型」の活用は雇用維持につながる傾向があります。

研究者は、キャリアの起点となる仕事が消失し、次世代が市場から排除されるリスクについて強い懸念を表明しています。
...
ハーネス・エンジニアリングは、Birgitta Boeckeler氏の着想に基づき、AIアシスタントが生成するコードが時間の経過とともにアーキテクチャや命名規則から逸脱する「ドリフト」を防ぐための実践体系です。主な構成要素は次の3つです。

- **コンテキスト・エンジニアリング**: AIに知識を与えるための専用基盤（HARNESS.md）の維持。
- **アーキテクチャ制約**: 「検証スロット」を設け、決定論的ツールやAIエージェントによってコードの妥当性をチェックする仕組み。
- **ガベージコレクション**: スケジュールに基づきコードベースの不整合を排除するプロセス。

本手法の特徴は「漸進的硬化（Progressive Hardening）」にあり、制約を「未検証」から「エージェントによるレビュー」、そして最終的には「決定論的な自動チェック（CIなど）」へと昇格させていく成熟度モデルを提唱しています。また、開発セッションの振り返り（/reflect）を通じてハーネス自体を自己改善させるループも組み込まれています。
...
本記事は、SemiAnalysisによるクローズドモデル（OpenAI、Anthropic）とオープンソースモデル（Meta、DeepSeek、Zhipu等）の能力格差に関する定量的分析である。LLMの進化を3つの時代に分けて検証している。

- **初期スケーリング時代（2022–2024）**: Llama-2がGPT-3.5に追いつくまで1年以上を要した。
- **推論時代（2024–2025）**: OpenAI o1に対しDeepSeek R1が約8.5ヶ月で追随し、格差が急速に縮小した。
- **エージェント時代（2025–現在）**: Claude Opus 4.5級のモデルに対し、Kimi K3やGLM-5が約5ヶ月で同等性能に到達している。

世代ごとにキャッチアップ期間が「半分」になる傾向（12ヶ月→8.5ヶ月→約5ヶ月）が確認された。一方で、ベンチマーク数値では測れない「製品としての統合性（ハーネス）」や安全性テストの期間が、クローズド勢の優位性を支える実質的な障壁として残ると指摘している。
...
インテルは次世代の「エージェンティックAI」環境に向けた3つの主要アーキテクチャの詳細を公開した。これらはIntel 18AプロセスやFoveros Direct 3D、UCIeなどの先端技術を基盤としている。

- **Diamond Rapids（次世代Xeon）**: 最大256コアと1.28GBの共有L3キャッシュを備え、新しいAPX/AMX拡張によりオーケストレーションを最適化。
- **Crescent Island（データセンターGPU）**: Xe3Pベースの32コアと最大480GBの広帯域メモリを搭載し、推論の経済性とトークン生成効率を劇的に向上。
- **Wildcat Lake（Intel Core Series 3）**: クライアント/エッジ向けSoC。メインストリーム製品として初めてUCIeを採用し、17 TOPSのNPUによりハイブリッドAIを実現。

これらにより、企業は現実的な電力・コスト制約の中で、自律的なAIエージェントを大規模に展開することが可能になる。
...
ハリウッドの第一線で活躍する脚本家、監督、プロデューサーたちが、時給12〜200ドルでAnthropicやOpenAIなどのAIモデルのトレーニングに従事している。ロサンゼルスの撮影日数が2021年から2025年にかけて48%減少するなど制作本数が激減するなか、多くのクリエイターが「自らの仕事を代替する技術」を育てるという苦渋の決断を迫られている実態を報じている。

具体的な作業は、脚本の評価、詳細な制作スケジュールの作成、音声文字起こしの精度向上など多岐にわたる。現場からは「自分の職業の墓を掘るためのシャベルを渡されたようだ」という悲痛な声が上がる一方、「AIはまだ人間の感情や機微を再現できておらず、単純な構成作業の補助に留まる」という冷静な見方もある。

Netflixが2026年までにタイトルの30%でAIを利用する見込みであるなど、業界構造の変革はすでに避けられない段階に入っている。
...
デザイン会社グッドパッチによる、AI活用のスピードを落とさずにリスクを管理する「守りのAI設計」に関する詳細レポートである。統制を「ブレーキ」ではなく「ガードレール」と定義し、以下の4つの柱で構成されている。

- **文書**: 抽象的な表現を排除し、自社固有の機密情報をレベル分けしてマトリクス化。
- **運用**: 申請制による停滞（関所）を避け、セキュリティ確認済みツールのホワイトリスト化を推進。
- **体制**: 推進チームと統制チームを分離し、AIエージェントを用いた『Agentic Management』により管理業務を効率化。
- **計測**: 投資を『システム費』ではなく『人件費』と捉え、浸透・投資・効果の3軸で経営指標化。

現場のスピードを削がずに安全性を担保する、実践的なAI統制のあり方を提示している。
