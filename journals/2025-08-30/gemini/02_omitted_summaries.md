...
https://gigazine.net/news/20250822-deepseek-v-3-1-released/

DeepSeekは、エージェントタスク向けに推論モードと非推論モードを両立させ、旧モデルより高速化したハイブリッドAIモデル「DeepSeek-V3.1」をリリースしました。
...
**Topics**: [[LLM, Agent AI, Function Calling, Performance Benchmarks, API Updates]]

中国のAI企業DeepSeekが、エージェント時代の第一歩と位置づけるハイブリッドAIモデル「DeepSeek-V3.1」をリリースしました。このモデルは、思考を行う「Think」モードと行わない「Non-Think」モードを一つのモデル内で両立させることで、従来のDeepSeek-R1-0528より高速な推論を実現しています。

Webアプリケーション開発者にとって重要な点は、APIが「deepseek-reasoner」(Thinkモード)と「deepseek-chat」(Non-Thinkモード)として提供され、両方とも128Kのコンテキスト長をサポートすることです。特に、ベータAPIで厳密な関数呼び出し(Strict Function Calling)に対応した点は、AIエージェントが外部ツールを正確に利用するワークフローを構築する上で極めて実用的です。これにより、開発者はAIによる自動化されたデータ取得やタスク実行の信頼性を高めることができます。

さらに、SWE-benchやTerminal-Benchといったコーディングおよびターミナル操作のベンチマークで高いスコアを記録し、複雑な検索タスクにおける複数ステップ推論能力と思考効率が大幅に向上しています。出力トークン数も削減されており、API利用コストの最適化にも寄与する可能性があります。モデルデータはHugging Faceで公開されており、長文コンテキスト拡張のための事前学習が施されたベースモデルも利用可能です。これらの進化は、WebアプリケーションにおけるAIエージェントの導入を加速させ、より高性能で効率的な自動化ツール開発を可能にするでしょう。
...
https://zenn.dev/mkj/articles/2a0b7f9788578d
