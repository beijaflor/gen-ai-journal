...
def export_omitted_sources(metadata: List[Dict[str, Any]]) -> None:
    ...
    if not omitted_summaries:
        # Still write the file (header only) so it always exists. A missing
        # file makes `git add workdesk/omitted_sources.md` abort the whole
        # staging — a recurring STEP_03b/05 gotcha.
        with open(OMITTED_FILE, "w", encoding="utf-8") as f:
            f.write("# Omitted Sources\n\n")
            f.write("<!-- Auto-generated by export_curation_flags.py -->\n")
            f.write("<!-- Total: 0 summaries (no summaries flagged for omission) -->\n")
        print(f"No summaries flagged for omission (wrote empty {OMITTED_FILE})")
        return
