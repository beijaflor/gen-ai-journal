...
| **standout_flag** | Exceptional content marker | Adds ⭐ marker in exports |
| **omit_flag** | Exclude from both journals (too specialized, low quality) | Exported to `omitted_sources.md` |
| **upvote_flag** | Curator endorsement (critical insight, high value) | Adds 👍 marker in exports |
| **downvote_flag** | Curator concerns (hype, shallow, misleading) | Adds 👎 marker in exports |
...
- [x] 003. https://example.com/unique-perspective ⭐ 👍
- [x] 012. https://example.com/niche-insight 👍
- [x] 025. https://example.com/alternative-view

```

**Markers:**
- `⭐` = standout_flag=true (exceptional content)
- `👍` = upvote_flag=true (curator endorsement)
- `👎` = downvote_flag=true (curator concerns)
...
- [ ] 042. https://example.com/too-specialized 👎
...
✅ Marked 42 summaries as published for journal date 2025-12-27
```

**Effect:**

- **State Transition:** Draft (workdesk) → Published
- **RLS Policy Change:**
...
summary_metadata
├── id                UUID           -- Primary key
├── summary_id        VARCHAR(10)    -- "001", "002", etc.
├── url               TEXT           -- Source URL
├── journal_date      DATE           -- NULL = workdesk, set = published
│
├── annex_flag        BOOLEAN        -- Consider for annex journal
├── standout_flag     BOOLEAN        -- Exceptional content (⭐)
├── omit_flag         BOOLEAN        -- Exclude from both journals
├── upvote_flag       BOOLEAN        -- Curator endorsement (👍)
├── downvote_flag     BOOLEAN        -- Curator concerns (👎)
│
├── ai_scores         JSONB          -- AI-generated scores (read-only in UI)
│   └── {signal, depth, unique, practical, antiHype, mainJournal, annexPotential, overall}
│
├── created_at        TIMESTAMPTZ    -- Record creation timestamp
├── updated_at        TIMESTAMPTZ    -- Last update timestamp
└── updated_by        UUID           -- FK to auth.users (audit trail)
...
- **Fix:** Check Supabase dashboard → Settings → API → Schema exposure
...
Browser UI (localhost:4321)
    ↓
SummaryEditControls.astro → supabase.upsertSummaryMetadata()
    ↓
...
export_curation_flags.py
    ├─ SELECT * FROM summary_metadata WHERE journal_date IS NULL
    ├─ Filter: annex_flag=true → curated_annex_journal_sources.md
    └─ Filter: omit_flag=true → omitted_sources.md
    ↓
...
Never manually edit or overwrite `ai_scores` field. It's read-only in the admin UI for good reason—preserves original AI judgment.
...
- Show 👍/👎 badges next to journal entries
- Display standout marker ⭐ for exceptional content
